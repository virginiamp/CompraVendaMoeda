...
#pragma endregion
	...
	private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
		...
		if (!String::IsNullOrEmpty(this->textBox1->Text)) {
			...
			label42->Text = L"";
			if (MR == 0) {
				label44->Text = L"valor não pode ser zero";
				label8->Text = L"Erro!";
			}
		}
		else {
		...
		if (!String::IsNullOrEmpty(this->textBox2->Text)) {
			...
			label43->Text = L"";
			if (M==0) {
				label45->Text = L"valor não pode ser zero";
				label8->Text = L"Erro!";
			}
		}
		else {
		...
		if ((controle1 == 0) && (controle2 == 0)) {
			if ((MR != 0) && (M != 0)){
				...
				else { label10->Text = Convert::ToString(valorVenda); }
				...
				else { label38->Text = Convert::ToString(valorizacao); }				
			}
			else {
				if (MR == 0) { 
					label44->Text = "Valor não pode ser zero";
					label8->Text = L"Erro!";
				}
				if (M == 0)  { 
					label45->Text = "Valor não pode ser zero"; 
					label8->Text = L"Erro!";
				}
			}
		}
		else { controle1 = controle2 = 0; }
	}
				...
				if (!String::IsNullOrEmpty(this->textBox6->Text)) {
					...
					if (montanteR != 0) {
						if (Regex::IsMatch(Convert::ToString(lucroR / montanteR), str)) {
							label26->Text = Regex::Replace(Convert::ToString((lucroR / montanteR)*100), str, "$1");
						}
						else {
							label26->Text = Convert::ToString((lucroR / montanteR)*100);
						} 
					}
				}
			}
